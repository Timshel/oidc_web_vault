# Changelog

# v2026.9.0-1

- Release based on `web-v2026.9.0`

# v2026.8.1-2

- Fix for Organization invite with SSO login

# v2026.8.1-1

- Release based on `web-v2026.8.1`

# v2026.8.0-1

- Release based on `web-v2026.8.0`
- Remove commit to delete onboarding can now use `ServerSettings.suppressOnboardingInterstitials` to disable it.

# v2026.7.1-1

- Release based on `web-v2026.7.1`

# v2026.7.0-1

- Release based on `web-v2026.7.0`

# v2026.6.4-2

- Firefox addon for AMO and fix `update_url`.

# v2026.6.4-1

- Release based on `web-v2026.6.4`

# v2026.6.3-1

- Release based on `web-v2026.6.3`
- Add `update_url` to extension manifest

# v2026.6.2-2

- Fix server settings

## v2026.6.2-1

- Release based on `web-v2026.6.2`
- Build Firefox and Chrome extension

## v2026.6.1-1

- Release based on `web-v2026.6.1`

## v2026.6.0-3

- Fix group external ID handling

## v2026.6.0-2

- Remove onboarding spam showed on each new device if the account is less than 30days old

## v2026.6.0-1

- Release based on `web-v2026.6.0`

## v2026.5.0-1

- Release based on `web-v2026.5.0`

## v2026.4.2-2

- Improve logic around hardcoded identifier

## v2026.4.2-1

- Release based on `web-v2026.4.2`

## v2026.4.1-3

- Changed hardcoded dummy identifier to `"00000000-01DC-01DC-01DC-000000000000"` to pass `isId` check
  (PR for [Vaultwarden](https://github.com/dani-garcia/vaultwarden/pull/7154)).

## v2026.4.1-2

- Changed hardcoded dummy identifier to `vaultwarden-dummy-oidc-identifier` (Same value used by [Vaultwarden](https://github.com/dani-garcia/vaultwarden/pull/7097)).

## v2026.4.1-1

- Release based on `web-v2026.4.1`

## v2026.3.1-1

- Release based on `web-v2026.3.0`

## v2026.3.0-1

- Release based on `web-v2026.3.0`

## v2026.2.1-1

- Release based on `web-v2026.2.1`

## v2026.2.0-1

- Release based on `web-v2026.2.0`

## v2026.1.1-2

- Fix org member invite

## v2026.1.1-1

- Release based on `web-v2026.1.1`

## v2026.1.0-1

- Release based on `web-v2026.1.0`

## v2025.12.2-1

- Release based on `web-v2025.12.2`

## v2025.12.1-2

- Fix merge leftovers

## v2025.12.1-1

- Release based on `web-v2025.12.1`

## v2025.12.0-2

- Fix remember device when using 2FA with the `override` client.
  This won't work when using multiple account in the same session.

## v2025.12.0-1

- Release based on `web-v2025.12.0`

## v2025.11.4-2

- Adjust defaults for login form settings to work with `Vaultwarden`.

## v2025.11.4-1

- Release based on `web-v2025.11.4`

## v2025.11.3-1

- Release based on `web-v2025.11.3`

## v2025.11.1-2

- Fix a crash when opening Org policies

## v2025.11.1-1

- Release based on `web-v2025.11.1`

## v2025.10.1-1

- Release based on `web-v2025.10.1`

## v2025.10.0-1

- Release based on `web-v2025.10.0`

## v2025.9.1-1

- Release based on `web-v2025.9.1`
- Changed hardcoded dummy identifier to `VW_DUMMY_IDENTIFIER_FOR_OIDC` (Same value used by [Vaultwarden](https://github.com/dani-garcia/vaultwarden/pull/6263)).

## v2025.9.0-2

- Disable extension onboarding spam

## v2025.9.0-1

- Release based on `web-v2025.9.0`

## v2025.8.1-1

- Release based on `web-v2025.8.1`

## v2025.8.0-1

- Release based on `web-v2025.8.0`

## v2025.7.2-1

- Release based on `web-v2025.7.2`

## v2025.6.2-1

- Release based on `web-v2025.6.2`

## v2025.6.1-1

- Release based on `web-v2025.6.1`

## v2025.6.0-1

- Release based on `web-v2025.6.0`

## v2025.5.1-1

- Release based on `web-v2025.5.1`

## v2025.5.0-1

- Release based on `web-v2025.5.0`

## v2025.4.2-2

- Allow to set External Id on Organization and Groups

## v2025.4.2-1

- Release based on `web-v2025.4.2`

## v2025.4.1-2

- Fix typo in providers

## v2025.4.1-1

- Release based on `web-v2025.4.1`

## v2025.3.1-4

- Fix SSO error display

## v2025.3.1-3

- Fix override landing page

## v2025.3.1-2

- Fix toggle login button

## v2025.3.1-1

- Release based on `web-v2025.3.1`

## v2025.3.0-1

- Release based on `web-v2025.3.0`

## v2025.2.2-1

- Release based on `web-v2025.2.2`

## v2025.1.2-4

- Move the dummy `identifier` to the `override` patch.
- During enrollment override the `idendifier` with the one returned by `auto-enroll-status`.

## v2025.1.2-3

- Fix the dummy `identifier` used.

## v2025.1.2-2

- Remove the now unnecessary org invite fix.

## v2025.1.2-1

- Release based on `web-v2025.1.2`

## v2025.1.1-1

- Release based on `web-v2025.1.1`

## v2025.1.0-2

- Add css class to 2FA providers

## v2025.1.0-1

- Release based on `web-v2025.1.0`

## v2024.12.1-3

- On login error redirect to the `loggedOut` default page

## v2024.12.1-2

- Add css class to easily hide the password field

## v2024.12.1-1

- Release based on `web-v2024.12.1`

## v2024.10.2-1

- Release based on `web-v2024.10.2`

## v2024.8.3-4

- Fix sso redirection to keep clean url

## v2024.8.3-3

- Fix confirm error message

## v2024.8.3-2

- Remove `Bitwarden Inc.` from footer

## v2024.8.3-1

- Switching `web-vaults` are now built, now rebased on top of https://github.com/bitwarden/clients
- Release based on `web-v2024.8.3`
