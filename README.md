...
Since `v2025.9.1-1` the clients are compatible with [Vaultwarden](https://github.com/dani-garcia/vaultwarden).

This generate two different `ẁeb-vault` versions :

- `button` default version, similar to [vw_web_builds](https://github.com/vaultwarden/vw_web_builds)
  - No change to the login flow, `email` input is still required
  - Set the SSO buttons as the default when (`SSO_ENABLED`)
- `override` which default landing page do not prompt for email :
...
Additionnaly this build the Firefox and Chrome extensions modified to not require email input (with the `override` patch).

For Firefox two version are available:

- unsigned: produced by the [release](.github/workflows/release.yml) action.
- tosign: produced by the [release](.github/workflows/release.yml) action but with the `update_url` pointing to `oidc_firefox_signed.xpi`.
- signed: manually uploaded after signing on amo.
...
\
This means that when logging in with a second user the stored token will be overridden and the remember device only work with this new session.

## Building the web-vault

To build the web-vault you need node `22` cf [.nvmrc](.nvmrc) and npm installed.
